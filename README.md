...
🛠️ **В разработке — v1.2: фоновая служба (Windows Service, LocalSystem).** Бэкап и
восстановление выполняются с нужными правами без открытого окна, а расписания
срабатывают, **даже когда в систему никто не вошёл** (GUI становится тонким клиентом
службы). Уже собрано и проходит обкатку — выйдет ближайшим релизом.
